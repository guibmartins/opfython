...
class Annoy(ApproximateNNSearch):
    ...
    def __init__(self, params: dict, n_jobs=-1):
...
class HNSW(ApproximateNNSearch):
    ...
    def __init__(self, params: dict):
        ...
        self.graph = hnswlib.Index(space=params.get('distance'), dim=params.get('n_features'))
    ...
    def fit(self, X: np.ndarray, save_file: bool = False):
        ...
        self.graph.init_index(
            ...
            ef_construction=self.params.get('ef') * self.params.get('C'),
            M=self.params.get('M'),
            random_seed=self.params.get('seed')
        )
    ...
    def load_index(self, path_file: str, max_samples: int):
        ...
        self.graph = hnswlib.Index(space=self.params.get('space'), dim=self.params.get('n_feats'))
        self.graph.load_index(path_file, max_elements=max_samples)
        self.graph.set_ef(max_samples)

    def set_ef(self, ef: int):
        self.graph.set_ef(ef * self.params.get('C'))
...
class KD_Tree(ApproximateNNSearch):
    ...
    def set_default_params(self):
        ...
        self.params.update({'leaf_size': 30, 'distance': 'minkowski'})
