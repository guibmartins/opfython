...
class ANNSubgraph(Subgraph):
    ...
    def __init__(self, X=None, Y=None, I=None, from_file=None):
        ...
        super(ANNSubgraph, self).__init__(X, Y, I, from_file)
            ...
            for j in range(self.n_nodes):
                ...
                if j != i:
                    ...
                    while cur_k > 0 and distances[cur_k] < distances[cur_k - 1]:
                        ...
                        # Swaps the neighbours indices from `k` and `k-1`
    ...
    def calc_pdf(self, n_neighbours, distance_function):
        """Calculates the probability density function for `k` neighbours.
    ...
    def build_arcs(self, k, ann_search):
        ...
        for i in range(self.n_nodes):
            ...
            neighbors_idx, distances = ann_search.query(self.nodes[i].features, k + 1)

            max_dist = float(np.max(distances[1:]))
            ...
            self.nodes[i].adjacency.extend(neighbors_idx[1:])

            self.nodes[i].adj_distances.extend(distances[1:])
